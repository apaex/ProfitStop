...
STOP_SIZE            = 50              -- Размер стопа в минимальных шагах цены (если 0, ставится только профит)
PROFIT_SIZE          = 50              -- Размер профита в минимальных шагах цены (если 0, ставится только стоп)
...
BALANCE_TYPE         = 2               -- Тип отображения баланса в терминале Quik (1 - в лотах, 2 - с учетом количества в лоте)
...
WORK_TIME            = {               -- Промежутки времени, когда бот работает (можно изменять/добавлять/удалять промежутки)
   [1] = {
      ['BEGIN']      = '10:00:05',     -- Сохранять значение начала работы на первом месте
      ['END']        = '13:59:55'
   },
   [2] = {
      ['BEGIN']      = '14:05:05',
